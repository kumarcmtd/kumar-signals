    ...
    <div className="flex flex-col items-center" style={{ width: SIZE }}>
      <svg width={SIZE} height={HEIGHT} viewBox={`0 0 ${SIZE} ${HEIGHT}`}>
        ...
        <g style={{ transformOrigin: `${CX}px ${CY}px`, transform: `rotate(${rotation}deg)`, transition: "transform 0.7s cubic-bezier(.34,1.4,.64,1)" }}>
          <line x1={CX} y1={CY} x2={CX} y2={CY - R * 0.84} stroke={color} strokeWidth={2.5} strokeLinecap="round" />
        </g>
      ...
      <div className="flex flex-col items-center -mt-1">
        ...
        <span className="text-[9px] font-black uppercase tracking-wide leading-tight" style={{ color }}>
          {label}
        </span>
      </div>
    </div>
...
export function VolatilityMeter({ symbol, variant = "light" }: { symbol: InstrumentSymbol; variant?: "light" | "dark" }) {
  ...
  return (
    <div className={`rounded-2xl p-2.5 ${dark ? "" : "bg-white border border-slate-100 shadow-sm"}`} style={dark ? { background: "#181A24", border: "1px solid rgba(255,255,255,.08)" } : undefined}>
      <p className={`text-[9px] font-bold uppercase flex items-center gap-1 mb-0.5 ${dark ? "text-white/50" : "text-slate-500"}`}>
        <GaugeIcon size={10} /> {DISPLAY_NAME[symbol] ?? symbol} Speed
      </p>
      <div className="flex items-center justify-center">
        <SpeedGauge score={reading.score} color={reading.color} label={reading.label} />
      </div>
      <div className={`flex justify-between text-[8px] px-2 mt-0.5 ${dark ? "text-white/30" : "text-slate-400"}`}>
        <span>Calm</span>
        <span>Extreme</span>
      </div>
      <div className={`space-y-0.5 mt-1.5 text-[9px] ${dark ? "text-white/50" : "text-slate-500"}`}>
        <p>
          ATR: <b className={dark ? "text-white/80" : "text-slate-700"}>₹{reading.atrValue}</b> ({reading.atrPct}%) · Last candle: <b className={dark ? "text-white/80" : "text-slate-700"}>₹{reading.lastRange}</b>
        </p>
        {reading.estPremiumSwing !== null && (
          <p>
            Est. premium swing/candle: <b className={dark ? "text-white/80" : "text-slate-700"}>±₹{reading.estPremiumSwing}</b>
          </p>
        )}
      </div>
    </div>
